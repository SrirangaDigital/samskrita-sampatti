        ...
        <div class="col-md-8">
            <div id="cd-sec-nav">
                <nav id="cd-sec-main-nav">
                    <ul>
                        <li><a class="years" href="<?=BASE_URL?>book/list?details.collection.category=Sahithya"><i class="fa fa-book-open"></i>All Works</a></li>
...
<div class="container mainpage mt-5">
    ...
    <?php if(isset($data['alphabet'])) {?>
        <p class="col-md-7 alphabet">
            <?php foreach ($data['alphabet'] as $letter) { ?><a class="letter" href="<?=BASE_URL?>listing/sahithya/<?=$letter?>?details.collection.category=Sahithya"><?=$letter?></a> <?php } ?>
        </p>
    <?php } ?>
    </div>
    <div class="row justify-content-center">
    <?php foreach ($data['values'] as $row) { ?>
        <?php if(isset($row['item'])) { ?>
                <div class="full-width-card col-md-3">
                    <h3 class="item"><a href="<?=$data['nextUrl']?><?=$row['item']?>?sahithya=<?=$data['filter']['sahithya']?>"><?=$row['item']?></a></h3>
