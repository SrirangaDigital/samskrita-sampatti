<div class="container-fluid back-gray card-slider subjects">
	<div class="row justify-content-center mb-4">
		<div class="col-md-3">
			<h3 class="title">Purana - Itihasa</h3>
		</div>
	</div>
	<div class="row justify-content-center mb-5 card-deck">
		<a href="<?=BASE_URL?>book/list?details.collection.category=Ithihasa Purana&details.collection.itihasa=Ramayana" class="card col-md-3">
			<img src="<?=STOCK_IMAGE_URL?>scene-01.jpg" class="card-img-top" alt="वेदवेदाङ्ग">
			<div class="card-body">
				<h5 class="card-title">Ramayana</h5>
				<p class="card-desc eng"></p>
			</div>
		</a>
		<a href="#" class="card col-md-3">
			<img src="<?=STOCK_IMAGE_URL?>scene-02.jpg" class="card-img-top" alt="वेदवेदाङ्ग">
			<div class="card-body">
				<h5 class="card-title">Mahabharata</h5>
				<p class="card-desc eng"></p>
			</div>
		</a>
		<a href="#" class="card col-md-3">
			<img src="<?=STOCK_IMAGE_URL?>scene-03.jpg" class="card-img-top" alt="वेदवेदाङ्ग">
			<div class="card-body">
				<h5 class="card-title">Bhagavata</h5>
				<p class="card-desc eng"></p>
			</div>
		</a>
	</div>
	<div class="row justify-content-center mb-3 card-deck">
		<a href="#" class="card col-md-3">
			<img src="<?=STOCK_IMAGE_URL?>scene-04.jpg" class="card-img-top" alt="वेदवेदाङ्ग">
			<div class="card-body">
				<h5 class="card-title">Purana</h5>
				<p class="card-desc eng"></p>
			</div>
		</a>
    
	</div>
</div>
